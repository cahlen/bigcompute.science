...
const EXPERIMENTS = [
  {
    ...
    status: "complete",
    summary: "Original v6 run reported zero uncovered denominators to 2.1e11, but this is strong evidence rather than certified proof because v6 emitted no no-overflow certificate. Hardened v6.1 currently certifies d <= 1e6 locally; full 210B v6.1 rerun pending.",
    key_results: { evidence_range: "2.1e11", certified_range: "1e6", failures_reported_v6: 0, delta: 0.836829443681208, D0_estimate: "3.4e10 (conditional)" },
  ...
  },
  {
    ...
    status: "complete",
    summary: "A={1,2,3} has exactly 27 exceptions through the completed 10^10 run. No simple sharp phase transition at Hausdorff dim = 1/2; dimension, transitivity, and representation distribution all matter.",
    key_results: {
      exceptions_A123: 27,
  ...
  },
  {
    ...
    status: "in-progress",
    summary: "GPU search for CF formulas for mathematical constants. 145B+ candidates exhausted through deg 5, zero confirmed transcendental hits. Deg 4 range [-5,5]: 26B candidates, only sqrt(2) (2 false-positive pi*ln(2) failed mpmath). Deg 5 running. v3 kernel: 29 compound expressions + zero-value filter.",
...
const FINDINGS = [
  ...
  },
  {
    ...
    claim: "Computer-assisted proof framework, not a completed proof. Original v6 brute-force run reported zero uncovered denominators to 2.1×10^11 but is not certified; v6.1 certifies d≤10^6. MOW threshold D₀≈3.4×10^10 is conditional on unresolved operator and constant-tracking gaps.",
    our_data: { D0_estimate: "3.4e10 (conditional)", evidence_range_v6: "2.1e11", certified_range_v6_1: "1e6", spectral_gap_delta: 0.836829, method: "MOW + finite-matrix arb interval arithmetic" },
    ...
    oeis_sequences: [],
    url: "https://bigcompute.science/findings/zaremba-conjecture-framework/",
  },
  {
    slug: "zaremba-spectral-gaps-uniform",
    title: "Congruence Spectral Gaps Are Uniform",
    claim: "Spectral gap σ_m ≥ 0.237 across all 1,214 square-free moduli to m=1999 in the finite computation. Property (τ) is computationally supported, not proved by this dataset.",
  ...
  },
  {
    ...
    title: "Transitivity Verified to p=17,389",
    claim: "The semigroup generated by {1,...,5} acts transitively on nonzero vectors for all tested primes p≤17,389. The all-prime Dickson argument remains provisional and needs independent verification in the correct ambient group.",
    our_data: { method: "finite BFS + provisional Dickson strategy", scope: "tested primes", verified_computationally_to: 17389 },
  ...
  },
  {
    ...
    title: "Exploratory Zaremba Generator BFS Depths",
    claim: "Exploratory BFS depths for Zaremba generators were computed for 172 primes to p=1021. Audit caveat: current kernel is not a certified SL2 Cayley diameter computation because of ambient-group and validation issues.",
    our_data: { max_prime: 1021, primes_tested: 172, status: "exploratory; corrected certificate needed" },
  ...
  },
  {
    ...
    claim: "5 digits including 1 beat 19 digits without 1 in Hausdorff dimension. dim_H(E_{1,2,3,4,5}) = 0.8368 > dim_H(E_{2,...,20}) = 0.7683.",
    our_data: { dim_12345: 0.836829, dim_2to20: 0.768314, subsets_computed: 1048575 },
  ...
  },
  {
    ...
    claim: "Zaremba representation counts are expected to grow like d^(2δ-1) ≈ d^0.674 by transfer-operator heuristics and available computations; this is evidence, not a standalone proof.",
  ...
  },
  {
    ...
    claim: "Complete density for all {1,k} pairs at 10^11 and {2,k} comparisons where logs are available. {1,k} decays sharply with k; stable candidate exception sets are observational and not proved finite.",
    our_data: { pairs_computed: 18, range: "10^10-10^11 depending on pair", density_12_1e11: "80.75%", density_13_1e11: "9.11%", stable_candidate_exception_sequence: "27, 64, 374, 1834, 7178" },
      ...
      const toolMap: Record<string, () => Promise<any>> = {
        ...
        suggest_experiment: async () => {
          const suggestions = [
            ...
            { priority: 2, experiment: "Extend A={1,2,3} 27-exception check with a completed 10^11/10^12 log", command: "./zaremba_density_gpu 1000000000000 1,2,3", gpu_hours: 8, impact: "HIGH — tests whether the stable 27-exception candidate persists; would not by itself prove closure.", area: "zaremba" },
